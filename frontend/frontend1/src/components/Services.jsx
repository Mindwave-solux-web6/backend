...
//   const emotionName = props.name;
//   const foundInfo = FeelingList.find((item) => item.name === emotionName);
...
//     try {
//       // Send diary content to your server for emotion analysis
//       const emotionResponse = await axios.post("http://127.0.0.1/api/flask", {
//         sentence: content,
//       });
//       setEmotionResult(emotionResponse.data);
...
//             {emotionResult && <Feelings name={emotionResult} />}
...
function Feelings(props) {
  const emotionName = props.name || props.emotionResult;
  const foundInfo = FeelingList.find((item) => item.name === emotionName);
...
const Services = (e) => {
  ...
  //    setResult(res.data);
  //  } catch (error) {
  //    console.error('감정 결과 가져오기 실패:', error);
  //  }
  //};
